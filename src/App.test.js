...
import { findByDataAttr, findByDataAttrWhenMounted, getStateClone, initialState } from './_utils';
import App from './App';

jest.mock('axios');
jest.mock('./_components/route-layouts/layout/layout', () => () => <div>Mock Aomponent</div>);
...
describe('App', () => {
  describe('Components Testing', () => {
    ...
    it('should have siteSettingContextProvider', () => {
      wrapper = setupWrapper(initialState);
      const component = findByDataAttr(wrapper, 'siteSettingContextProvider').first();
      expect(component).toHaveLength(1);
    });

    it('should have userContextProvider', () => {
      wrapper = setupWrapper(initialState);
      const component = findByDataAttr(wrapper, 'userContextProvider').first();
      expect(component).toHaveLength(1);
    });
